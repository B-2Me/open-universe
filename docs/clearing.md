...
Standard Brownian motion (a Wiener process) generates new positions just by adding random noise: `x_{t+\Delta t} = x_t + \text{noise}`. Because a zero-mass particle can change direction instantly, it creates sharp, infinitely jagged angles that do not exist in reality.
